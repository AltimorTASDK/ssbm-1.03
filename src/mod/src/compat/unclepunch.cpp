...
[[gnu::constructor]] static void check_unclepunch()
{
	...
	PATCH_LIST(
		...
		// lwz r4, 0x2C(r3)
		std::pair { AS_235_SpotDodge_YoshiCheck+0x10, 0x8083002Cu },
		// mr r4, r24
		std::pair { CSS_Setup+0x2230,                 0x38980000u },
		// cmpw r3, r0
		...
		// mr r3, r28
		std::pair { Match_InitPlayers+0x1D0,          0x387C0000u },
		// lbz r0, 0x24D0(r31)
		std::pair { Match_InitPlayers+0x254,          0x881F24D0u },
		// mr r3, r27
		std::pair { Player_Respawn+0x80,              0x387B0000u },
		...
		// li r3, 0x4800
		std::pair { Scene_Initialize+0x54,            0x38604800u },

		// Disable UP's built in rumble toggle
		std::pair { CSS_PlayerThink+0x638,            0x889F0004u },
		// Disable UP's built in rumble off on unplug
		std::pair { HSD_PadRenewMasterStatus+0x98,    0x8819000Au },
