...
static bool has_item(const Player *player)
{
	return player->held_item != nullptr;
}
...
{
	if (!has_item(player) && get_player_config(player).c_up != cstick_type::smash)
		return false;
...
{
	if (!has_item(player) && get_player_config(player).c_horizontal != cstick_type::smash)
		return false;
...
{
	if (!has_item(player) && get_player_config(player).c_down != cstick_type::smash)
		return false;
...
{
	if (has_item(player) || get_player_config(player).c_up != cstick_type::tilt)
		return false;
...
{
	if (has_item(player) || get_player_config(player).c_horizontal != cstick_type::tilt)
		return false;
...
{
	if (has_item(player) || get_player_config(player).c_down != cstick_type::tilt)
		return false;
